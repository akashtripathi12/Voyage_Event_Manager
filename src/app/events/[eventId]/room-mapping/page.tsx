...
export default function RoomMappingPage() {
  ...
  // 3. AND event is NOT finalized
  const isAllocating = (hasAllocations || showAllocationUI) && !isFinalized;
  ...
  const unallocatedFamilies = useMemo(() => {
    const allocatedFamilyIds = new Set(allocatedFamilies.map((a) => a.family_id));
  ...
  const finalizeMutation = useMutation({
    ...
    onSuccess: () => {
        flashMessage("success", "Event finalized. Room mapping is now locked.");
        queryClient.invalidateQueries({ queryKey: ["allocations", eventId] });
    },
    onError: (err: any) => flashMessage("error", err.message),
  });
  ...
  // Placeholder for agent check - in real app derives from token/context
  const isAgent = true; 
    ...
    <div className="min-h-screen bg-neutral-50 relative">
      ...
      <div className="bg-white border-b border-neutral-200 px-8 py-6 sticky top-0 z-10 shadow-sm">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
                <h1 className="text-2xl font-black text-neutral-900 tracking-tight flex items-center gap-3">
                    ...
                    <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide ${
                        !isFinalized ? "bg-green-100 text-green-700" : "bg-neutral-100 text-neutral-600"
                    }`}>
                        {isFinalized ? "FINALIZED" : "ACTIVE"}
                    </span>
                </h1>
                <Link href={`/events/${eventId}`} className="text-sm text-neutral-500 hover:text-neutral-900 mt-1 block">
                    ← Back to Dashboard
                </Link>
            </div>
            ...
            <div className="flex items-center gap-3">
                ...
                {/* Start Allocation Button (UX Only) */}
                {!isFinalized && !hasAllocations && !showAllocationUI && isAgent && (
                    <button
                ...
                {!isFinalized && (isAllocating || hasAllocations) && (
                    <button
                ...
                {!isFinalized && (isAllocating || hasAllocations) && (
                    <button
                        onClick={() => { if(confirm("Lock and finalize rooms? This will prevent further changes.")) finalizeMutation.mutate(); }}
                    ...
                    >
                        {finalizeMutation.isPending ? "Locking..." : "Finalize & Lock"}
                    </button>
                )}
      ...
      {isFinalized && (
          <div className="bg-amber-50 border-b border-amber-100 px-8 py-3 text-center text-amber-800 text-sm font-medium flex items-center justify-center gap-2">
              ...
              Event is finalized. Allocations are locked.
          </div>
      )}

      {/* Main Content */}
      {(isAllocating || hasAllocations || isFinalized) ? (
          ...
          <div className="max-w-7xl mx-auto px-8 py-24 text-center">
              <div className="bg-white p-12 rounded-2xl shadow-sm border border-neutral-100 max-w-lg mx-auto">
                ...
                <p className="text-neutral-600 mb-8">
                    You have {guests.length} guests and {inventory.reduce((sum, r) => sum + r.total, 0)} rooms available.
                    Start assigning rooms to families manually or use auto-allocation.
                </p>
                <button
                    onClick={() => setShowAllocationUI(true)}
